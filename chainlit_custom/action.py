"""
This file is a custom action for chainlit.
We are using this custom action to send actions to the frontend.
Custom action is with default value (None) for forId, so action do not need to be connected to message.
"""
import uuid
...
@dataclass
class CustomAction(DataClassJsonMixin):
